...
public class Chat_With_ChatGPT_Model implements Chat_With_ChatGPT_Contract.Model {
    ...
    public void sendMessageToServer(String message, Action successCallBack, Action failCallBack) {
        ...
        ChatGPT_Message userMessage = new ChatGPT_Message("user", message);
        List<ChatGPT_Message> messagesHistoryForRequest = FormMessageHistoryForRequest(userMessage); //Формируем историю сообщений
        ChatGPT_SendMessage_Request request = new ChatGPT_SendMessage_Request(messagesHistoryForRequest); //Формируем запрос на сервер
        messagesHistory.add(userMessage); //Добавляем в список сообщений сообщение от пользователя
        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        Log.println(Log.INFO, "ChatGPT API Connection", gson.toJson(request));
